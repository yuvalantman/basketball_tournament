...
create table if not exists public.groups (
  ...
  display_options jsonb not null default '{
    ...
    "best_worst": false, "archetype": false, "group_strength": false
  }'::jsonb,
  created_at      timestamptz not null default now()
);
...
create table if not exists public.gamedays (
  ...
  -- Opt-in self-join cap. Null (the default) = manager-only add, exactly the
  -- original flow; set = regular members can self-join up to this many
  -- participants via join_gameday() below, then spill to the waitlist.
  max_players int null check (max_players is null or max_players > 0),
  created_at timestamptz not null default now()
);
...
-- Atomic self-join: the `for update` lock below serializes concurrent joins
-- for THIS gameday only (other gamedays are unaffected) — the second caller
-- always re-reads a fresh count after the first transaction commits, so
-- gameday_participants can never exceed max_players even under a race.
-- Capacity enforcement can't live in RLS (no write policies exist on
-- gameday_participants/gameday_waitlist — every write goes through the
-- admin client) and can't be two separate Supabase-JS calls either (that
-- has a real TOCTOU race), so the whole decision has to be one Postgres
-- function. Called only via the admin client from app/actions/gameday.ts.
...
-- Atomic self-leave, mirroring app/actions/gameday.ts's removeParticipant
-- (same team-slot-vacate + waitlist-head-promotion logic, just self-service
-- and lock-protected instead of manager-only).
