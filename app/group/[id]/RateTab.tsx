      ...
      <Card className="bg-[var(--surface-2)]">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-sm text-[var(--muted)]">
            {rated.size}/{others.length} rated
          </span>
        </div>
        <p className="text-xs text-[var(--muted)]">
          Rating never closes — rate whoever you like, whenever, and change your mind anytime.
          Only {overallParam(group.sport)?.label ?? "one field"} is required per rating; the rest
          are optional.
        </p>
    ...
    <Card className="p-0 overflow-hidden">
      <button onClick={onToggle} className="w-full flex items-center gap-3 p-3 text-left">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-[var(--muted)]">{isRated ? "Tap to edit your rating" : "Tap to rate"}</div>
        </div>
        {isRated ? (
          <Badge className="bg-green-500/15 border-green-500/40 text-green-400">✓ Rated</Badge>
        ) : (
      ...
      {isOpen && (
        <div className="border-t border-[var(--border)] p-4 space-y-3">
          ...
          {overall && <p className="text-[11px] text-[var(--muted)]">* required</p>}
          {error && <p className="text-red-400 text-sm">{error}</p>}
          <Button className="w-full" onClick={save} disabled={!ready || saving || !loaded}>
            {saving ? <Spinner /> : isRated ? "Save changes" : "Save rating"}
          </Button>
        </div>
      )}
    </Card>
